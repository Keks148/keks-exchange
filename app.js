(() => {
  ...
  const OPTIONS = [...BANKS, ...CRYPTO, ...WALLETS];
  ...
  const I18N = {
    UA: { swap:"Обмін", rules:"Правила", faq:"FAQ", acc:"Акаунт",
      youGive:"Віддаєте", youGet:"Отримуєте", result:"Ви отримаєте",
      ...
      netFee:"Комісія мережі", noRate:"Курс недоступний — перевіряємо джерела"
    },
    EN: { swap:"Exchange", rules:"Rules", faq:"FAQ", acc:"Account",
      youGive:"You give", youGet:"You get", result:"You receive",
      ...
      netFee:"Network fee", noRate:"Rate unavailable — checking sources"
    },
    PL: { swap:"Wymiana", rules:"Zasady", faq:"FAQ", acc:"Konto",
      youGive:"Oddajesz", youGet:"Otrzymujesz", result:"Otrzymasz",
      ...
      netFee:"Opłata sieci", noRate:"Brak kursu — sprawdzamy źródła"
    }
  };
  ...
  const state = {
    ...
    whitebit: { ts: 0, map: new Map() },
    nbu: { ts: 0, rates: { USD: null, EUR: null } },

    lock: { active:false, until:0, lockedValuesUAH: new Map() },
    order: null,
  };
  ...
  // ✅ ИСПРАВЛЕНО: USDT обрабатываем ДО любых вычислений через USDT
  function valueUAH_baseCoin(baseCoin){
    ...
    const usdtUah = getMid("USDT_UAH"); // ✅ без рекурсии
  ...
  function applyRounding(getValue, amount){
    ...
    if (opt.type === "bank" || opt.type === "wallet") return Math.floor(amount);
  ...
  function activateLock(){
    ...
    state.lock.until = nowMs() + 3*60*1000;
    state.lock.lockedValuesUAH = new Map();

    const baseSet = new Set(["UAH","USDT","USD","EUR"]);
  ...
  function renderOrderBlock(){
    ...
    if (!state.order) return `<div class="meta">${t.updated}: ${fmtTime()}</div>`;
  ...
  function renderScreen(){
    ...
    if (state.tab !== "swap") {
      if (state.tab === "rules") {
        screen.innerHTML = `<div style="font-weight:1000;font-size:26px;margin:2px 0 10px;">${t.rules}</div>
          <div style="color:var(--muted);font-weight:800;line-height:1.4;">(заглушка)</div>`;
        return;
      }
      if (state.tab === "faq") {
      ...
      if (state.tab === "acc") {
        screen.innerHTML = `<div style="font-weight:1000;font-size:26px;margin:2px 0 10px;">${t.acc}</div>
          <div style="color:var(--muted);font-weight:800;line-height:1.4;margin-bottom:10px;">(заглушка)</div>
          <div class="row">
    ...
    screen.innerHTML = `
      ...
      <div class="row">
        <div class="picker">
          ...
          ${buildSelect(state.give, "assetGive")}
        </div>
      </div>
      ...
      <div class="row">
        <div class="picker">
          ...
          ${buildSelect(state.get, "assetGet")}
        </div>
      </div>
    ...
    $("#assetGive").addEventListener("change", (e)=>{
      state.give = e.target.value;
      renderScreen();
    });

    $("#assetGet").addEventListener("change", (e)=>{
  ...
  function start(){
    render();
    refreshRates();
    setInterval(refreshRates, 30_000);
  }
